...
async function getFavoritePostService(idRenter) {
  ...
  let info_owner = await User.findOne({ _id: idRenter }).populate(
    "favorite_post"
  );
...
// Tìm kiếm thông tin
async function searchAllPostService(
  n,
  address_room,
  kind_room,
  general_owner,
...
let allPostRecentService = () => {
  return PostRoom.find({ status: "active" })
    .sort("-createdAt")
    .populate("idOwner")
    .limit(6);
};
...
let allPostProvinceService = () => {
  return Promise.all([
    ...
    PostRoom.find({
      status: "active",
      address_room: /Đà Nẵng/,
    }).countDocuments(),
    PostRoom.find({
      status: "active",
      address_room: /Đồng Nai/,
    }).countDocuments(),
    PostRoom.find({
      status: "active",
      address_room: /Bình Dương/,
    }).countDocuments(),
  ]);
};
